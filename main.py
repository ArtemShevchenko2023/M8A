...
async def ресурсы(ctx):
    resources = [
        "Статья: Как бороться с глобальным потеплением - [ссылка](https://example.com/article)",
        "Организация: Greenpeace - [ссылка](https://example.com/greenpeace)",
        "Руководство: 10 способов снизить углеродный след - [ссылка](https://example.com/guide)"
    ]
...
async def хелп(ctx):
    aboba = ("**&пр - команда приветствия, использование &пр**""\n"
            ...
            "**&цикл - команда, повторяющая последнюю вклученную песню, использование &цикл (кол-во повторений в цифрах)**""\n"
